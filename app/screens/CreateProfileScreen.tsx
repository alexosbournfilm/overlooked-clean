...
const HUMAN_FILMMAKING_POLICY_VERSION = 'human-filmmaking-v1';
...
const [roleLoadError, setRoleLoadError] = useState<string | null>(null);
  ...
  const validateStage = () => {
    ...
    if (stage === 'review' && !aiPolicyAccepted) {
      Alert.alert(
        'Human filmmaking policy',
        'Please confirm that you understand AI filmmaking is not allowed on Overlooked.'
      );
      return false;
    }
  ...
  if (!aiPolicyAccepted) {
    ...
    Alert.alert(
      'Human filmmaking policy',
      'Please tick the box confirming that you understand AI filmmaking is not allowed on Overlooked.'
    );
    return;
  }
...
if (!userId) {
  ...
  return;
    ...
    const { data: savedProfile, error } = await upsertProfileRobust({
      ...
      ai_policy_accepted_at: new Date().toISOString(),
      ai_policy_version: HUMAN_FILMMAKING_POLICY_VERSION,
...
if (Platform.OS === 'web' && typeof window !== 'undefined') {
  ...
  const finishBlocked = isLastStep && !aiPolicyAccepted;
      ...
      <Text style={[styles.avatarFallbackText, { color: theme.text }]}>Add Profile Image</Text>
              ...
              {stage === 'review' && (
                <>
                  ...
                  <View
                    style={[
                      styles.humanPolicyCard,
                      {
                        backgroundColor: theme.elevated,
                        borderColor: aiPolicyAccepted ? theme.accentBorder : theme.borderStrong,
                      },
                    ]}
                  >
                    <View style={styles.humanPolicyHeader}>
                      <View style={[styles.humanPolicyIcon, { backgroundColor: theme.accentSoft }]}>
                        <Ionicons name="film-outline" size={18} color={theme.accent} />
                      </View>
                      <View style={styles.humanPolicyHeaderText}>
                        <Text style={[styles.humanPolicyKicker, { color: theme.accent }]}>
                          Human filmmaking policy
                        </Text>
                        <Text style={[styles.humanPolicyTitle, { color: theme.text }]}>
                          AI filmmaking is not allowed here.
                        </Text>
                      </View>
                    </View>

                    <Text style={[styles.humanPolicyBody, { color: theme.sub }]}>
                      Overlooked is designed to preserve human filmmaking. Films uploaded or
                      submitted here must be made by people, not generated by AI.
                    </Text>

                    <Text style={[styles.humanPolicyWarning, { color: theme.text }]}>
                      If you are caught uploading or submitting AI-made films, your account will
                      be terminated.
                    </Text>

                    <TouchableOpacity
                      onPress={() => setAiPolicyAccepted((value) => !value)}
                      style={[
                        styles.humanPolicyCheckboxRow,
                        {
                          backgroundColor: aiPolicyAccepted ? theme.accentSoft : theme.input,
                          borderColor: aiPolicyAccepted ? theme.accentBorder : theme.border,
                        },
                      ]}
                      activeOpacity={0.86}
                      disabled={loading}
                    >
                      <View
                        style={[
                          ...
                          aiPolicyAccepted && styles.checkboxSelected,
                          aiPolicyAccepted && {
                            backgroundColor: theme.accent,
                            borderColor: theme.accent,
                          },
                      ...
                      <Text style={[styles.humanPolicyCheckboxText, { color: theme.text }]}>
                        I understand that Overlooked is for human filmmaking, AI films are not
                        allowed, and my account will be terminated if I upload or submit AI-made
                        films.
                      </Text>
                    </TouchableOpacity>
                  </View>
            ...
            <View style={styles.reviewActions}>
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  { backgroundColor: theme.accent },
                  (loading || finishBlocked) && { opacity: 0.6 },
                ]}
                disabled={loading || finishBlocked}
                activeOpacity={0.9}
              >
                {loading ? (
